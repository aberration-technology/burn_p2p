...
use burn_p2p_ui::{BrowserAppClientView, BrowserAppStaticBootstrap};
use js_sys::Reflect;
use wasm_bindgen::JsValue;
use web_sys::window;

const EDGE_STORAGE_KEY: &str = "burn-p2p.edge-base-url";
...
pub(crate) fn normalize_edge_base_url(
    value: &str,
    bootstrap: &BrowserAppStaticBootstrap,
) -> String {
    let normalized = value.trim();
    if normalized.is_empty() {
        return resolve_edge_base_url(bootstrap);
    }
    resolve_edge_candidate(normalized, bootstrap)
}

pub(crate) fn load_saved_edge_base_url() -> Option<String> {
    let storage = storage()?;
    let value = storage.get_item(EDGE_STORAGE_KEY).ok().flatten()?;
...
pub(crate) fn save_edge_base_url(edge_base_url: &str) {
    if let Some(storage) = storage() {
        let _ = storage.set_item(EDGE_STORAGE_KEY, edge_base_url);
    }
}

pub(crate) fn load_saved_surface() -> Option<burn_p2p_ui::BrowserAppSurface> {
    let storage = storage()?;
    ...
    if trimmed.is_empty() {
        ...
        Some(burn_p2p_ui::BrowserAppSurface::from_key(trimmed))
    }
}

pub(crate) fn save_surface(surface: burn_p2p_ui::BrowserAppSurface) {
    if let Some(storage) = storage() {
