//! OIDC-backed identity connector implementations for burn_p2p.
#![forbid(unsafe_code)]
...
#[derive(Debug)]
/// Represents an OIDC identity connector.
pub struct OidcIdentityConnector(ProviderMappedIdentityConnector);

impl OidcIdentityConnector {
    /// Creates a new value.
    pub fn new(
    ...
    /// Returns a copy configured with a JWKS endpoint for id_token validation.
...
impl IdentityConnector for OidcIdentityConnector {
...
mod tests {
    ...
    use crate::OidcIdentityConnector;

    #[test]
    fn oidc_connector_issues_oidc_sessions() {
        let now = Utc::now();
        let connector = OidcIdentityConnector::new(
            ...
            BTreeMap::from([(
                ...
                StaticPrincipalRecord {
                    claims: PrincipalClaims {
                        ...
                        granted_scopes: BTreeSet::from([ExperimentScope::Discover]),
        ...
        let login = connector
            .begin_login(LoginRequest {
                ...
                requested_scopes: BTreeSet::from([ExperimentScope::Discover]),
            })
            .expect("login");
