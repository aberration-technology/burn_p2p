...
use burn_p2p::{
    BrowserMode, CallbackPayload, CompiledFeatureSet, ContributionReceipt, DatasetViewId,
    EdgeFeature, ExperimentDirectoryEntry, ExperimentDirectoryPolicyExt,
    ExperimentOptInPolicy, ExperimentResourceRequirements, ExperimentScope,
    ExperimentVisibility, HeadDescriptor, HeadEvalReport, HeadEvalStatus, IdentityConnector,
    LeaseId, LoginRequest, MetricTrustClass, MetricValue, NodeEnrollmentRequest, OverlayChannel,
    OverlayTopic, PeerId, PeerRole, PeerRoleSet, PeerWindowMetrics, PeerWindowStatus,
    PrincipalId, ProfileMode, ReducerCohortMetrics, ReducerCohortStatus,
    ReducerLoadAnnouncement, SocialMode, TrustedIssuer, WorkloadId,
};
...
use burn_p2p_core::{
    ArtifactLiveEventKind, AuthProvider, BackendClass, ClientPlatform, ContentId, NetworkId,
    RevocationEpoch,
};
...
fn metrics_routes_export_snapshots_ledger_and_head_views() {
    let storage = burn_p2p::StorageConfig::new(
        tempdir()
            .expect("artifact metrics tempdir")
            .keep(),
    );
    ...
    assert_eq!(head_reports.as_array().expect("head reports array").len(), 1);
    assert_eq!(
    ...
    let portal_body = response_body(&portal_html);
    assert!(portal_body.contains("Canonical quality"));
    assert!(portal_body.contains("Global progress"));
    assert!(portal_body.contains("Decentralized dynamics"));
    assert!(portal_body.contains("Peer-window distributions"));
    assert!(portal_body.contains("Branch candidates"));
    assert!(portal_body.contains("Reducer disagreement"));
    assert!(portal_body.contains("Network health"));
    assert!(portal_body.contains("exp-metrics loss"));
    assert!(portal_body.contains("head-candidate"));
    assert!(portal_body.contains("/metrics/peer-windows/exp-metrics/rev-metrics/head-base"));
    assert!(portal_body.contains("Operator hint:"));
    assert!(portal_body.contains("/metrics/candidates/exp-metrics"));
    assert!(portal_body.contains("/metrics/disagreements/exp-metrics"));
    assert!(portal_body.contains("Artifacts"));
    assert!(portal_body.contains("latest/serve"));
    assert!(portal_body.contains("/artifacts/export"));
    ...
    tokio::runtime::Builder::new_current_thread()
        ...
        .block_on(async move {
            ...
            assert_eq!(
                snapshots[0].manifest.experiment_id.as_str(),
                "exp-metrics"
            );

            let experiment_snapshots = client
                .fetch_metrics_snapshots_for_experiment(&burn_p2p::ExperimentId::new(
                    "exp-metrics",
                ))
                .await
            ...
            let experiment_catchup = client
                .fetch_metrics_catchup_for_experiment(&burn_p2p::ExperimentId::new(
                    "exp-metrics",
                ))
                .await
            ...
            assert_eq!(peer_window_detail.summary.base_head_id.as_str(), "head-base");
            ...
            let experiment_aliases = client
                .fetch_artifact_aliases_for_experiment(&burn_p2p::ExperimentId::new(
                    "exp-metrics",
                ))
                .await
            ...
            let export_job = client
                .request_artifact_export(&burn_p2p_publish::ExportRequest {
            ...
            assert_eq!(export_job_view.status, burn_p2p_core::ExportJobStatus::Ready);
            ...
            assert_eq!(streamed_live_event.kind, ArtifactLiveEventKind::ExportJobReady);
            assert_eq!(
                streamed_live_event
            ...
            let ticket = client
                .request_artifact_download_ticket(&burn_p2p_publish::DownloadTicketRequest {
            ...
            assert!(
                ticket
                    .download_path
                    .starts_with("/artifacts/download/")
            );
            assert_eq!(
