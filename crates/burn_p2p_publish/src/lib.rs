...
mod tests {
    ...
    #[test]
    fn open_recovers_from_corrupt_registry_json() {
        let root = tempdir().expect("tempdir");
        let artifact_store = FsArtifactStore::new(root.path().join("cas"));
        artifact_store.ensure_layout().expect("layout");
        let (_descriptor, head) = sample_artifact(&artifact_store, "artifact-a", "head-a");
        ...
        let recovered = PublicationStore::open(root.path()).expect("recover corrupt registry");
        let view = recovered
            .head_view(std::slice::from_ref(&head), &reports, &head.head_id)
            .expect("head view");
        ...
        let registry_json = std::fs::read_to_string(crate::registry::state_path(root.path()))
            .expect("registry json should be persisted after recovery");
        assert!(!registry_json.trim().is_empty());
    }
