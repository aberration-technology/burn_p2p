...
pub use burn_p2p_swarm::{
    ...
    MicroShardFetchRequest, MicroShardFetchResponse, MicroShardProviderRecord,
...
pub use config::{
    ...
    ControlHandle, DatasetConfig, IdentityConfig, NodeConfig, NodeRuntimeState,
    NodeTelemetrySnapshot, RuntimeStatus, SlotAssignmentState, SlotRuntimeState, StorageConfig,
...
use runtime_support::{
    ...
    runtime_topology_peers, runtime_validators, set_effective_limit_profile,
    update_announces_from_connected_snapshots, update_norm_stats, verify_snapshot_admission,
};
...
fn dedupe_peer_ids(peers: impl IntoIterator<Item = PeerId>) -> Vec<PeerId> {
...
impl<P> RunningNode<P> {
        ...
        P: RuntimeProject<B>,
    {
        ...
        let project = &mut self
            ...
            .project;
        let device = project.runtime_device();
        let model = if let Some((_, base_head)) = current_head.as_ref() {
        ...
        let mut limit_profile = effective_limit_profile(
            &storage,
            experiment,
            &local_peer_id,
            &node_config,
        ...
        let window_id = next_window_id(&storage, experiment)?;
        let overlays = experiment.overlay_set()?;
...

        {
            ...
            snapshot.set_primary_slot_state(SlotRuntimeState::FetchingShards(assignment.clone()));
        }
        let throughput_sample_started_at = Utc::now();
        let cached_microshards = ShardCache::new(storage.dataset_cache_dir())
            .fetch_lease_microshards(&registration, &microshard_plan, &lease.lease)?;
        let batches = project.load_batches(&lease.lease, &cached_microshards)?;
        ...
        };
        {
            ...
            snapshot.set_primary_slot_state(SlotRuntimeState::Training(assignment.clone()));
        }
        ...
        let artifact = project.materialize_model_artifact(
            ...
            ArtifactKind::FullHead,
            head_id.clone(),
            current_head.as_ref().map(|(_, head)| head.head_id.clone()),
            &store,
        )?;
        ...
        let observed_throughput = ObservedThroughputUpdate {
            measured_work_units: lease.selection.estimated_work_units.max(1),
            elapsed_seconds: (throughput_sample_finished_at - throughput_sample_started_at)
                .num_seconds()
                .max(1) as u64,
            completed_windows: window_id.0.min(u64::from(u32::MAX)) as u32,
            sampled_at: throughput_sample_finished_at,
        };
        limit_profile = calibrator.rebudget(&limit_profile, observed_throughput)?;
        persist_limit_profile(&storage, experiment, &limit_profile)?;
        set_effective_limit_profile(&self.telemetry, Some(limit_profile));
        let head = HeadDescriptor {
            head_id: head_id.clone(),
            study_id: experiment.study_id.clone(),
            experiment_id: experiment.experiment_id.clone(),
            revision_id: experiment.revision_id.clone(),
            artifact_id: artifact.artifact_id.clone(),
            parent_head_id: current_head.as_ref().map(|(_, head)| head.head_id.clone()),
        ...
        persist_json(
            storage.scoped_receipt_path(&contribution.receipt_id),
            &contribution,
        )?;
        persist_json(storage.scoped_head_path(&head.head_id), &head)?;
        persist_window_id(&storage, experiment, window_id)?;
        self.update_runtime_state(
            NodeRuntimeState::PublishingUpdate,
            Some(SlotRuntimeState::Publishing(assignment.clone())),
        );
        self.control.publish_merge_window(MergeWindowAnnouncement {
            overlay: overlays.heads.clone(),
            merge_window: merge_window.clone(),
            announced_at: Utc::now(),
        })?;
        self.control
            .publish_reducer_assignment(ReducerAssignmentAnnouncement {
                overlay: overlays.heads.clone(),
                assignment: reducer_assignment.clone(),
                announced_at: Utc::now(),
            })?;
        self.publish_artifact_from_store(&artifact.artifact_id)?;
        self.control.publish_update(UpdateEnvelopeAnnouncement {
            ...
            update: UpdateAnnounce {
                ...
                delta_artifact_id: artifact.artifact_id.clone(),
                sample_weight: contribution.accepted_weight,
                quality_weight: (1.0 / (1.0 + metric_quality(&contribution.metrics).abs()))
                    .max(0.01),
                norm_stats: update_norm_stats(&contribution.metrics),
                receipt_root: ContentId::derive(&[contribution.receipt_id.as_str()])?,
                receipt_ids: vec![contribution.receipt_id.clone()],
                providers: vec![local_peer_id.clone()],
                announced_at: Utc::now(),
            },
        })?;
        self.control.publish_head(HeadAnnouncement {
        ...
        P: RuntimeProject<B>,
    {
        ...
        let mut candidate_heads = Vec::<(PeerId, HeadDescriptor)>::new();
        for (peer_id, snapshot) in &snapshots {
            for announcement in &snapshot.head_announcements {
                ...
                if current_head.as_ref().map(|(_, head)| head.head_id.clone())
                    != announcement.head.parent_head_id
                {
                    continue;
                }
        ...
        let base_model = if let Some((_, base_head)) = current_head.as_ref() {
            match store.load_manifest(&base_head.artifact_id) {
                Ok(descriptor) => project.load_model_artifact(
                    project.init_model(&device),
                    &descriptor,
        ...
        let mut candidate_models =
            Vec::<(PeerId, HeadDescriptor, MetricReport, f64, f64, P::Model)>::new();
        ...
        for (peer_id, head) in candidate_heads {
            ...
            let quality = updates
                ...
                .unwrap_or_else(|| {
                    (1.0 / (1.0 + metric_quality(&evaluation.metrics).abs())).max(0.01)
                });
        ...
        let Some(fallback_best_index) = candidate_models
            .iter()
            .enumerate()
            .min_by(|(_, left), (_, right)| {
                metric_quality(&left.2.metrics)
                    .partial_cmp(&metric_quality(&right.2.metrics))
        ...
        let merged_model = {
            let merge_candidates = candidate_models
                .iter()
                .map(|(peer_id, head, _, sample_weight, quality_weight, model)| {
                    MergeModelCandidate {
                        peer_id,
        ...
        let (source_peer_id, merged_head, evaluation) = if let Some(merged_model) = merged_model {
            ...
            let source_peer_id = candidate_models
                ...
                .max_by(|left, right| {
                    (left.3 * left.4)
                        .partial_cmp(&(right.3 * right.4))
                        .unwrap_or(std::cmp::Ordering::Equal)
                })
                .map(|candidate| candidate.0.clone())
                .unwrap_or_else(|| local_peer_id.clone());
            (source_peer_id, merged_head, evaluation)
        } else {
            let (source_peer_id, merged_head, evaluation, _, _, _) =
                candidate_models.swap_remove(fallback_best_index);
            (source_peer_id, merged_head, evaluation)
        };
        ...
        let aggregate_merge_plan = MergePlan {
            study_id: experiment.study_id.clone(),
            experiment_id: experiment.experiment_id.clone(),
            revision_id: experiment.revision_id.clone(),
            base_head_id: base_head_id.clone(),
            policy: merge_policy.clone(),
        ...
        let aggregate_record = AggregateArtifactRecord {
            ...
            merge_plan: aggregate_merge_plan,
            inputs: updates
                .iter()
        ...
        store.store_prebuilt_artifact_bytes(
            &aggregate_artifact.descriptor,
            &aggregate_artifact.bytes,
        )?;
        let aggregate = AggregateEnvelope {
            aggregate_id: aggregate_id.clone(),
            study_id: experiment.study_id.clone(),
            experiment_id: experiment.experiment_id.clone(),
            revision_id: experiment.revision_id.clone(),
            ...
            reducer_peer_id: local_peer_id.clone(),
            ...
            providers: std::iter::once(local_peer_id.clone())
        ...
        let reduction_certificate = ReductionCertificate {
            ...
            validator_quorum: topology_policy.promotion_policy.validator_quorum.max(1),
            cross_checked_reducers: merge_window
                .reducers
                .iter()
                .take(usize::from(topology_policy.reducer_replication.max(1)))
                .cloned()
                .collect(),
            issued_at: Utc::now(),
        };

        let contribution = ContributionReceipt {
            receipt_id: ContributionReceiptId::new(format!(
                "{}-validated-{}-{}",
                experiment.experiment_id.as_str(),
        ...
        let merge_certificate = MergeCertificate {
            ...
            experiment_id: experiment.experiment_id.clone(),
            revision_id: experiment.revision_id.clone(),
            base_head_id: contribution.base_head_id.clone(),
        ...
        persist_head_state(&storage, experiment, &merged_head)?;
        persist_json(storage.scoped_head_path(&merged_head.head_id), &merged_head)?;
        store.pin_head(&merged_head.head_id)?;
        store.pin_artifact(&merged_head.artifact_id)?;
        store.pin_artifact(&aggregate_artifact.descriptor.artifact_id)?;
        self.update_runtime_state(
            NodeRuntimeState::PublishingUpdate,
            Some(SlotRuntimeState::Publishing(assignment.clone())),
        );
        ...
        self.control.publish_reducer_load(ReducerLoadAnnouncement {
            overlay: experiment.overlay_set()?.telemetry.clone(),
        ...
        self.control.publish_artifact(
            aggregate_artifact.descriptor.clone(),
            aggregate_artifact
                .descriptor
                .chunks
                .iter()
                .map(|chunk| {
                    ...
                    ArtifactChunkPayload {
                        artifact_id: aggregate_artifact.descriptor.artifact_id.clone(),
                        chunk: chunk.clone(),
                        bytes: aggregate_artifact.bytes[start..end].to_vec(),
                        generated_at: Utc::now(),
                    }
                })
                .collect(),
        )?;
        self.publish_artifact_from_store(&merged_head.artifact_id)?;
        self.control.publish_merge(MergeAnnouncement {
            overlay: experiment.overlay_set()?.heads,
            certificate: merge_certificate.clone(),
            announced_at: Utc::now(),
        })?;
        self.control.publish_head(HeadAnnouncement {
            overlay: experiment.overlay_set()?.heads,
            provider_peer_id: Some(local_peer_id.clone()),
            head: merged_head.clone(),
            announced_at: Utc::now(),
        })?;
...
pub mod prelude {
    ...
    pub use crate::{
        ...
        NodeTelemetrySnapshot, P2pProject, P2pProjectFamily, P2pWorkload, PatchClass, PatchOutcome,
        PatchSupport, PatchValue, PlannedLease, Project, ProjectBackend, ReleaseManifest,
        ReleasePolicy, ReputationDecision, ReputationEngine, ReputationObservation,
        ReputationPolicy, ReputationState, RevisionCompatibility, RevisionSpec, RoleSet,
        RunningNode, RuntimePatch, RuntimeProject, RuntimeStatus, SecurityError,
...
mod tests {
    ...
    fn issue_local_test_peer_auth(
        ...
        let node_keypair = super::runtime_support::resolve_identity(
    ...
    fn create_adaptive_runtime_dataset(project: &AdaptiveBudgetProject) {
        ...
        for entry in &manifest.entries {
            let bytes = format!("{}\n", 1 + (entry.ordinal % shard_count)).into_bytes();
            fs::write(project.dataset_root.join(&entry.locator), bytes)
                .expect("write adaptive shard");
        }
    }
        ...
        wait_for(
            ...
            || {
                ...
                snapshot.status == crate::RuntimeStatus::Running
                    ...
                    && snapshot.control_plane.control_announcements.iter().any(|announcement| {
                        matches!(
